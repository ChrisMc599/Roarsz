...
function zerif_pricing_header_title_function() {

	$zerif_pricing_title = get_theme_mod('zerif_pricing_title',__('YOUR pricing','zerif-lite'));

	if( !empty($zerif_pricing_title) ):
		echo '<h2 class="dark-text">'.wp_kses_post( $zerif_pricing_title ).'</h2>';
	elseif ( is_customize_preview() ):
		echo '<h2 class="dark-text zerif_hidden_if_not_customizer"></h2>';
	endif;
}

function zerif_pricing_header_subtitle_function() {

	$zerif_pricing_subtitle = get_theme_mod('zerif_pricing_subtitle',__('Prove that you have real people working for you, with some nice looking profile pictures and links to social media.','zerif-lite'));

	if( !empty($zerif_pricing_subtitle) ):

		echo '<div class="section-legend">'.wp_kses_post( $zerif_pricing_subtitle ).'</div>';
